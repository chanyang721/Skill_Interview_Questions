...
멀티스레트를 사용하는 경우 공유 자원으로 인한 문제 해결을 위해 동기화에 신경을 써야합니다. 싱글 스레드는 이 동기화에 신경쓰지 않아도 되기 때문에 멀티 스레드보다 빠르다고 알고 있습니다. 

#### 싱글 스레드와 멀티 스레드 차이점은 무엇인가요.
프로세스 내부에 스레드는 작업 흐름의 단위입니다. 따라서 싱글스레드는 작업 흐름이 하나이며, 멀티는 작업의 단위가 두개 이상을 갖고 있습니다. 작업의 흐름의 단위가 하나인 싱글 스레드는 하나의 작업이 띁난 뒤에 두번째 작업을 시작하는 동기적 흐름을 가지고 있는 반면, 멀티 스레드는 스레드의 개수만큼 작업을 비동기적으로 진행할 수 있다는 차이점이 있습니다.
