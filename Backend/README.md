...
* 특히 node.js는 libuv 라이브러리로 Non-blocking I/O와 단일 스레스 방식의 이벤트 루프를 사용하여 통해 높은 처리 성능을 가지고 있는게 특징입니다.
...
* REST의 가장 중요한 특징은 각 요청(주문)은 그 "요청 자체로 어떤 기능을 하는지 추론이 가능해야한다"라는 원칙을 가지고 있습니다. 이를 위해 API 요청을 구성하는 URL, HTTP Method, Response Status에 각각 규칙을 만들어 알맞게 적용한다면, 그 API를 RESTful API라고 합니다.
