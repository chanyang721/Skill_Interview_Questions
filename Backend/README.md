...
* 비동기적인 작동처럼 보이는 동시성을 웹 애플리케이션에 부여하기 위해 자바스크립트의 V8엔진에 있는 Call Stack이라는 하나의 스레드와 자바스크립트의 런타임 중 하나인 웹 브라우저에서 여러 스레드를 동시에 사용하여 동시성을 구현하게 되는데요. 이벤트 루프는 작업들이 Call Stack으로 들어가는 우선 순위를 결정해주는 역할을 하기 때문에 동시성을 부여하기 위한 핵심적인 기능을 하고 있습니다.
