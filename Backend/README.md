...
* 이벤트 루프는 싱글스레드 기반 언어인 자바스크립트에 동시성을 부여하기 위한 개념입니다.
* 자바스크립트는 싱글스레드 기반 언어이기 때문에 한번에 하나의 작업만 할 수 있습니다 하지만, 이벤트 루프라는 또 다른 싱글 스레드와 상호작용하며 비동기 처리를 수행하게 되기 때문에 동시성을 부여받습니다. 

* 이벤트 루프는 싱글 스레드 기반 언어인 자바스크립트에 동시성을 부여하기 위한 개념입니다. 동시성을 부여하기 위해 자바스크립트의 V8엔진 내부의 콜스택이라는 하나의 스레드와 런타임인 크롬의 하나의 스레드를 사용하여 동시성을 구현하였습니다.
* 
...
* 스레드는 프로세스 내부의 작업 흐름의 단위입니다. 따라서 싱글스레드는 작업 흐름이 하나이며, 멀티는 작업의 단위가 두개 이상을 갖고 있습니다. 
...
* 해당 프로그램을 실행시키는데 필요한 npm의 모듈들과 버전을 명시해놓은 파일로 협업 시 반드시 필요한 모듈들을 정확하게 명시하여 손쉽게 프로그램을 실행하기 위한 모듈들을 다운받을 수 있도록 하는 역할을 하고 있습니다.
